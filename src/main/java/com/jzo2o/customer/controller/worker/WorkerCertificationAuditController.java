...
public class WorkerCertificationAuditController {
    ...
    @PostMapping
    @ApiOperation("提交认证申请")
    public void submitCertification(@RequestBody WorkerCertificationAuditAddReqDTO workerCertificationAuditAddReqDTO) {
        workerCertificationAuditAddReqDTO.setServeProviderId(UserContext.currentUserId());
        workerCertificationAuditService.submitCertification(workerCertificationAuditAddReqDTO);
    }
    ...
    public RejectReasonResDTO rejectReason() {
        return workerCertificationAuditService.rejectReason(UserContext.currentUserId());
    }
}
